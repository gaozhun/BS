...
def test(obj_names, mvtec_path, checkpoint_path, base_model_name):
    ...
    for obj_name in obj_names:
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            display_image = gray_rec[0].transpose(0, 2).transpose(0, 1).detach().cpu().numpy()[:,:,::-1]
            display_gt_image = gray_batch[0].transpose(0, 2).transpose(0, 1).detach().cpu().numpy()[:,:,::-1]
            display_out_mask = out_mask_sm[0][1:].transpose(0, 2).transpose(0, 1).detach().cpu().numpy()[:,:,::-1]
            display_in_mask = true_mask[0].transpose(0, 2).transpose(0, 1).detach().cpu().numpy()[:,:,::-1]
            visualize(
                display_image = display_image,
                display_gt_image = display_gt_image,
                display_out_mask = display_out_mask,
                display_in_mask = display_in_mask,
            )
